...
resource "google_compute_security_policy" "security_policy" {
  ...
  # Adaptive Protection Config (Layer 7 DDoS Defense)
  dynamic "adaptive_protection_config" {
    for_each = var.enable_layer7_ddos_defense ? ["adaptive_protection_config"] : []
    content {
      layer_7_ddos_defense_config {
        enable          = var.layer7_ddos_defense_enable
        rule_visibility = var.layer7_ddos_defense_rule_visibility
      }
    }
  }
...
##### Global Security Rules Block IP addresses

resource "google_compute_security_policy_rule" "security_rules" {
  provider        = google-beta
  for_each        = local.is_global && var.security_rules != null ? { for x in var.security_rules : x.priority => x } : {}
  project         = var.project_id
...
resource "google_compute_security_policy_rule" "custom_rules" {
  provider        = google-beta
  for_each        = local.is_global && var.custom_rules != null ? { for x in var.custom_rules : x.priority => x } : {}
  project         = var.project_id
  security_policy = google_compute_security_policy.security_policy[0].name
  ...
  # Ignore expr_options computed by GCP
  lifecycle {
    ignore_changes = [
...
##### Global Preconfigured WAF Rules

resource "google_compute_security_policy_rule" "pre_configured_rules" {
  provider        = google-beta
  for_each        = local.is_global ? var.pre_configured_rules : {}
  project         = var.project_id
  security_policy = google_compute_security_policy.security_policy[0].name
...
resource "google_compute_region_security_policy" "security_policy" {
  ...
  # Note: Adaptive Protection Config is NOT supported in Regional policies
...
##### Regional Security Rules Block IP addresses
...
resource "google_compute_region_security_policy_rule" "security_rules" {
  ...
  project         = var.project_id
  region          = var.region
  security_policy = google_compute_region_security_policy.security_policy[0].name
...
##### Regional Preconfigured WAF Rules
